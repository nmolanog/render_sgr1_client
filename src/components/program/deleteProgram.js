...
export default function DeleteProgram(props) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const responseNumEnroll = await fetch(`https://rendersgr1server-production.up.railway.app/program/enrollmentsnum/${program.id}`,
                {
                    method: "GET",
    ...
    const handleDelete = async (e) => {
        ...
        try {
            ...
            const response = await fetch(`https://rendersgr1server-production.up.railway.app/program/deactivate/${program.id}`,
                {
                    method: "PUT",
                    headers: { "Content-Type": "application/json" },
                    credentials: "include"
